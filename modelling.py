...
def main(args=None):
    ...
    if active_run:
        logger.info('Using existing MLflow run: %s', active_run.info.run_id)
        run_context = active_run
    else:
        logger.info('Starting new MLflow run')
        run_context = mlflow.start_run(run_name=f'SVC_{datetime.now():%Y%m%d_%H%M%S}')
    ...
    try:
        ...
        # Hanya end run jika kita yang membuatnya (bukan dari mlflow run)
        if not active_run:
            mlflow.end_run()
