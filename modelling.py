...
dagshub.init(repo_owner="angerbit", repo_name="SMSML_angger_hanggara", mlflow=True)
...
def get_tracking_uri() -> str:
    ...
    # allow embedding username/token (for DagsHub or other hosted trackers)
...
def main(args=None):
    ...
    with mlflow.start_run(run_name=f'LinearSVC_{datetime.now():%Y%m%d_%H%M%S}'):
        ...
        model_path = run_dir / 'linear_svc_model.pkl'
